...
> **Important:** Always use `npm run release` to create releases. Never manually bump the version in `package.json`, manually edit `CHANGELOG.md`, or manually create git tags. `release-it` handles all of this automatically.

The project uses [release-it](https://github.com/release-it/release-it) to automate releases. The configuration is in `.release-it.json`. When you run `npm run release`, it will:

1. Bump the version in `package.json` (prompts you for patch/minor/major)
2. Auto-generate changelog entries in [CHANGELOG.md](CHANGELOG.md) from merged PR labels
3. Create a git commit and tag (`v{version}`)
4. Push the commit and tag to the remote
5. Create a GitHub release

#### Prerequisites

Set the `GITHUB_AUTH` environment variable with a [personal access token](https://docs.github.com/en/github/authenticating-to-github/creating-a-personal-access-token) that has the `repo` scope:

```bash
export GITHUB_AUTH="ghp_your_token_here"
```

#### PR labels

[CHANGELOG.md](CHANGELOG.md) is generated by [@release-it-plugins/lerna-changelog](https://github.com/release-it-plugins/lerna-changelog). For changelog entries to appear, merged pull requests need one of these labels:
...
`release-it` will guide you through the rest interactively.
