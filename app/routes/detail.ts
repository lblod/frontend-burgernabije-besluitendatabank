...
export default class DetailRoute extends Route {
  ...
  async model(params: any) {
    ...
    let longestArrayLength = Math.max(
      agendaItem.handledBy.get("hasVotes").toArray()[0]?.hasProponents.toArray()
        .length || 0,
      agendaItem.handledBy.get("hasVotes").toArray()[0]?.hasOpponents.toArray()
        .length || 0,
      agendaItem.handledBy.get("hasVotes").toArray()[0]?.hasAbstainers.toArray()
        .length || 0
    );
    ...
    console.log(formattedTableVote);
